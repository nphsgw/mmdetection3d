...
def init_model(config, checkpoint=None, device="cuda:0"):
    ...
    # configファイルのmodel定義を渡して、モデルを構築する。
...
def show_det_result_meshlab(
    data, result, out_dir, score_thr=0.0, show=False, snapshot=False
):
    ...
    show_result(
        points, None, show_bboxes, out_dir, file_name, show=show, snapshot=snapshot
    )
...
def show_result_meshlab(
    ...
    if task in ["det", "multi_modality-det"]:
        file_name = show_det_result_meshlab(
            data, result, out_dir, score_thr, show, snapshot
        )

    if task in ["seg"]:
        file_name = show_seg_result_meshlab(
            data, result, out_dir, palette, show, snapshot
        )
